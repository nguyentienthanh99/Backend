...
exports.getAllBanh = function(callbackQuery){
    connect();
    connection.query("SELECT kichthuoc_banh.MaBanh,SUM(ct_phieunhap.SL) As SL,SUM(ct_phieunhap.SLTon) AS SLTon,banh.TenBanh,loaibanh.MaLoaiBanh,nhacungcap.TenNCC,loaibanh.TenLoaiBanh,loaibanh.TrangThai AS TrangThaiLoaiBanh,banh.AnhSP,banh.HinhDang,banh.MoTa FROM kichthuoc_banh,ct_phieunhap,banh,loaibanh, phieunhap,nhacungcap WHERE ct_phieunhap.MaKT_Banh=kichthuoc_banh.MaKT_Banh and banh.MaBanh=kichthuoc_banh.MaBanh and banh.MaLoaiBanh=loaibanh.MaLoaiBanh AND phieunhap.MaPN =ct_phieunhap.MaPN AND phieunhap.MaNCC = nhacungcap.MaNCC AND ct_phieunhap.TrangThai=1 AND banh.TrangThai = 1 AND (NOW() BETWEEN ct_phieunhap.NSX AND ct_phieunhap.HSD) AND SL > 0 AND SLTon > 0 GROUP BY kichthuoc_banh.MaBanh",function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
...
exports.getDetailBanh= function(MaBanh,callbackQuery){
    connect();
    connection.query("SELECT kichthuoc_banh.MaBanh,SUM(ct_phieunhap.SL) As SL,SUM(ct_phieunhap.SLTon) AS SLTon,banh.TenBanh,loaibanh.MaLoaiBanh,nhacungcap.TenNCC,loaibanh.TenLoaiBanh,banh.AnhSP,banh.HinhDang,banh.MoTa FROM kichthuoc_banh,ct_phieunhap,banh,loaibanh ,phieunhap, nhacungcap WHERE ct_phieunhap.MaKT_Banh=kichthuoc_banh.MaKT_Banh and kichthuoc_banh.MaBanh= "+MaBanh+" and banh.MaLoaiBanh=loaibanh.MaLoaiBanh AND banh.MaBanh = "+MaBanh+" AND ct_phieunhap.TrangThai=1 AND banh.TrangThai = 1 AND (NOW() BETWEEN ct_phieunhap.NSX AND ct_phieunhap.HSD) AND phieunhap.MaPN = ct_phieunhap.MaPN AND phieunhap.MaNCC=nhacungcap.MaNCC GROUP BY kichthuoc_banh.MaBanh",function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
...
//các bánh cùng NCC
exports.getBanhCungNCC = function(MaNCC,callbackQuery){
    connect();
    connection.query("SELECT kichthuoc_banh.MaBanh,SUM(ct_phieunhap.SL) As SL,banh.TenBanh,loaibanh.TenLoaiBanh, banh.AnhSP FROM kichthuoc_banh,banh,loaibanh,ct_phieunhap,phieunhap,nhacungcap WHERE ct_phieunhap.MaKT_Banh=kichthuoc_banh.MaKT_Banh and banh.MaBanh=kichthuoc_banh.MaBanh and banh.MaLoaiBanh=loaibanh.MaLoaiBanh AND ct_phieunhap.TrangThai=1 AND loaibanh.TrangThai=1 AND phieunhap.MaPN= ct_phieunhap.MaPN AND nhacungcap.MaNCC = phieunhap.MaNCC AND nhacungcap.MaNCC = "+MaNCC+" AND (NOW() BETWEEN ct_phieunhap.NSX AND ct_phieunhap.HSD) GROUP BY kichthuoc_banh.MaBanh",function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
...
exports.getAllHoaDonXuat= function(callbackQuery){
    connect();
    connection.query("SELECT hoadon.MaHD,hoadon.MaKH,khachhang.TenKH,hoadon.TrangThaiGiaoHang,hoadon.TrangThaiThanhToan,hoadon.TrangThaiXacThuc,hoadon.created_at AS NgayTaoHD,hoadon.GhiChu FROM hoadon INNER JOIN khachhang ON hoadon.MaKH = khachhang.MaKH INNER JOIN ct_hoadon ON hoadon.MaHD = ct_hoadon.MaHD WHERE hoadon.TrangThai=1 AND khachhang.TrangThai =1 AND ct_hoadon.TrangThai = 1 GROUP BY hoadon.MaHD ORDER BY `hoadon`.`created_at` DESC",function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
...
//xóa hóa đơn xuất
exports.xoaHDX= function(MaHD,callbackQuery){
    connect();
    connection.query("UPDATE hoadon SET hoadon.TrangThai=0 WHERE hoadon.MaHD ="+MaHD,function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
...
exports.getBanhChuaKM = function(callbackQuery){
    connect();
    connection.query("SELECT banh.MaBanh,banh.TenBanh,khuyenmai.MaKM,khuyenmai.ThoiGianBatDau,khuyenmai.ThoiGianKetThuc FROM banh INNER JOIN kichthuoc_banh ON kichthuoc_banh.MaBanh = banh.MaBanh INNER JOIN ct_phieunhap ON kichthuoc_banh.MaKT_Banh = ct_phieunhap.MaKT_Banh LEFT JOIN khuyenmai_banh ON khuyenmai_banh.MaBanh = banh.MaBanh LEFT JOIN khuyenmai ON khuyenmai_banh.MaKM = khuyenmai.MaKM WHERE (NOW() BETWEEN ct_phieunhap.NSX AND ct_phieunhap.HSD) GROUP BY banh.MaBanh",function(err, results, _fields){
        if(!err){
                callbackQuery(results);
            }else console.log(err);
        })
